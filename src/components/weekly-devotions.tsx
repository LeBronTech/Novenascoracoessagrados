...
export default function WeeklyDevotions() {
  ...
  return (
    <div className="flex justify-center gap-2 md:gap-4 my-6">
      <TooltipProvider>
        {weeklyDevotions.map((devotion, index) => {
          const IconComponent = iconComponents[devotion.icon];
          const isCurrentDay = index === currentDay;
          return (
            <Tooltip key={devotion.day}>
              <TooltipTrigger asChild>
                <div
                  className={cn(
                    'devotion-item group flex flex-col items-center justify-center w-16 h-20 md:w-20 md:h-24 p-2 rounded-full border-2 border-gray-300/50 bg-gray-100/50 backdrop-blur-sm cursor-pointer',
                    isCurrentDay && 'active-day'
                  )}
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className="relative">
                    {IconComponent && <IconComponent className="devotion-icon w-6 h-6 md:w-8 md:h-8 text-gray-600" />}
                    {devotion.alert && (
                  ...
                  <span className="mt-1 text-xs md:text-sm font-bold text-center text-gray-700">{devotion.day}</span>
                </div>
              </TooltipTrigger>
